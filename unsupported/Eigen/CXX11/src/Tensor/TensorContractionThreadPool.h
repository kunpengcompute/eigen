...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>, ThreadPoolDevice> :
  ...
  void evalProductImpl(Scalar* buffer, DoneCallback done) const {
    ...
    if (may_use_kgemm) {
      ...
      const Index m_blocks = divup(m, kgemm_mr);
      ...
      if (m_tasks < num_threads) {
        const Index max_n_tasks = divup(n, kgemm_nr);
        n_tasks = numext::mini<Index>(max_n_tasks, divup<Index>(num_threads, m_tasks));
      }
      ...
      bm = divup<Index>(m_blocks, m_tasks) * kgemm_mr;
      const Index n_blocks = divup(n, kgemm_nr);
      bn = divup<Index>(n_blocks, n_tasks) * kgemm_nr;
      bk = k;
    ...
    defined(EIGEN_KUNPENG_KSPLIT_CHUNK_SIZE)
      ...
      shard_by_col = false;
      const Index output_tasks = divup(m, bm) * divup(n, bn);
