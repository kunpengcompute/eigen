...
All translation units that include Eigen tensor headers and participate in linking the same program should use the same macro configuration. When it is undefined or set to 0, the open-source Eigen implementation is used.
...
When the conditions are not met, it automatically falls back to Eigen's open-source packing and GEBP.
